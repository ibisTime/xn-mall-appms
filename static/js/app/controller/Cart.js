                    ...
                    if (response.success) {
                        ...
                        if (lists.length) {
                            ...
                            lists.forEach(function(cl) {
                                ...
                                html += '<li class="ptb8 plr10 clearfix b_bd_b p_r" code="' + cl.code + '" saleP="' + cl.product.price2 + '" cnyP="' + (cl.product.price3 || 0) + '">' +
                                    '<div class="wp100 p_r z_index0 over-hide">' +
                                ...
                                if (cl.status == "2") {
                                    ...
                                    html += '<div class="fl wp40 tc pl32 p_r c-img-l overhide">' +
                                        '<div class="cart-cont-left"><div class="radio-tip1 ab_l0"><i></i></div></div>' +
                                        '<a class="cart-center-img-a" href="../operator/buy.html?code=' + cl.productCode + '">';
                                }
                                html += '<img class="center-img1" src="' + base.getImg(cl.product.advPic) + '"/>' +
                                    '</a>' +
                                    '</div>' +
                                    '<div class="fl wp60 pl12">' +
                                    '<p class="t_323232 s_12 line-tow">' + cl.product.name + '</p><p class="t_f64444 s_12">';
                                ...
                                html += '</p>' +
                                    ...
                                    '<input class="buyCount tc w60p s_13 lh32" type="text" value="' + cl.quantity + '"/>' +
        ...
        function addListeners() {
                ...
                Ajax.post("808042", {json:param2})
                    .then(function(response) {
                        ...
                        if (response.success) {
                            ...
                            if (flag) {
                                $("#totalAmount").text(base.formatMoneyD(new_total)).data('price', new_total);
                                $("#totalCnyAmount").text(base.formatMoneyD(new_cnyTotal)).data('price', new_cnyTotal);
                            }
                        } else {
            ...
            $("#allChecked").on("click", function() {
                ...
                if (flag) {
                    ...
                    $("#totalAmount").text(base.formatMoneyD(t)).data('price', t);
                    $("#totalCnyAmount").text(base.formatMoneyD(t1)).data('price', t1);
                    //否则把页面底部总价置为0
                } else {
            ...
            $("#od-ul").on("click", "li[code] .cart-cont-left", function(e) {
                ...
                if (isChecked) {
                    ...
                    var ori_total = +$("#totalAmount").data('price');
                    var ori_cnyTotal = +$("#totalCnyAmount").data('price');
                    var new_total = ori_total + infos[$li.index()][0];
                    ...
                    var new_cnyTotal = ori_cnyTotal - infos[$li.index()][1];
                    $("#totalAmount").text(base.formatMoneyD(new_total)).data('price', new_total);
                    $("#totalCnyAmount").text(base.formatMoneyD(new_cnyTotal)).data('price', new_total);
                }
            });
            //确认删除框点击确认按钮
